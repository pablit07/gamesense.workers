...
async function drill_completionSummary(data, db, modifyHeader) {
	...
	if (data.filters.minDate && !data.paginate) {
		let cacheQuery = {report: 'drill_completionSummary', minDate: new Date(data.filters.minDate), team_name: data.filters.team_name};
	...
	if (cache) {
		...
		if (!data.paginate) {
			await db.collection("drill_reports").insertOne({minDate:new Date(data.filters.minDate), maxDate:data.filters.maxDate ? new Date(data.filters.maxDate) : new Date(moment().format('YYYY-MM-DD HH:mm')), team_name: data.filters.team_name, report:'drill_completionSummary',responses:responses});
		}
	}
...
async function drill_completionDetail(data, db, modifyHeader, applyDataFormat=x=>x) {
...
module.exports.drill_completionSummary = drill_completionSummary;
module.exports.drill_completionDetail = drill_completionDetail;
