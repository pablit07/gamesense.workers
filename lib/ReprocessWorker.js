...
class Task extends Worker {
	...
	async myTask(msgContent, msg, conn, ch, db) {
		...
		await currentSet.forEach(async r => {
			try {
				...
				if (i === 200) {
					Object.assign(r, {
						afterTask_responseMessage: {skip: msgContent.skip + 200, limit: msgContent.limit},
						afterTask_responseRmqQueueName: this.getAfterTaskResponseRmqQueueName(),
						afterTask_complete: msgContent.afterTask_complete
					});
				}
				// console.info('Fake send message', this.getExchange(), this.getRoutingKey());
				Publisher.publish(r, this.getExchange(), config.messageBroker.connectionString, Amqp, this.getRoutingKey(), {});
		...
		if (i < 200 && msgContent.afterTask_complete) {
	...
	async runOnce() {
		setTimeout(() => {
			Publisher.publishQ({skip: 0, limit: 200, afterTask_complete: true}, this.getAfterTaskResponseRmqQueueName(), config.messageBroker.connectionString, Amqp, {autoDelete:this.autoDelete});
		}, 3000)
		await this.run();
	}
}
